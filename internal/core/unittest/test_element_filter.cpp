...
TEST(ElementFilterSelectivity, StringNotEqualExpr) {
    ...
    auto mods = {2, 5, 10, 20, 33, 100};
    for (int mod : mods) {
        std::cout << "=== Testing with selectivity " << ((1.0 / mod) * 100)
                  << "% (modulo=" << mod << ") ===" << std::endl;
        // Step 5: Test with element-level filter using string not equal
...
TEST(ElementFilterSelectivity, IntRangeExpr) {
    ...
    for (int i = 0; i < raw_data.raw_->fields_data_size(); i++) {
        ...
        if (field_data->field_id() == int_array_fid.get()) {
            ...
            for (int row = 0; row < N; row++) {
                ...
                for (int elem = 0; elem < array_len; elem++) {
                    int64_t value = row * array_len + elem;
    ...
    // Step 5: Load STL_SORT index for int array field
    {
        // Flatten array data for indexing
        std::vector<int64_t> flat_int_data(N * array_len);
        for (size_t row = 0; row < N; row++) {
            for (int elem = 0; elem < array_len; elem++) {
                flat_int_data[row * array_len + elem] = row * array_len + elem;
            }
        }

        auto scalar_index =
            GenScalarIndexing<int64_t>(N * array_len, flat_int_data.data());
        ...
        // predicate: id % mod == 0 (selectivity = 1/mod)
        {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                for (auto j = 0; j < search_result->seg_offsets_.size(); j++) {
                    ...
                    // Verify element value is in range (10, 100)
                    int64_t element_value = doc_id * array_len + elem_idx;
                    ASSERT_GT(element_value, 10)
                        << "Element value should be > 10";
                    ASSERT_LT(element_value, 100)
                        << "Element value should be < 100";
                }
            }
        }
    }
