    ...
    get_col(FieldId field_id) const {
        ...
        for (auto i = 0; i < raw_->fields_data_size(); i++) {
            ...
            if constexpr (std::is_same_v<T, VectorArray>) {
                auto ret_data = reinterpret_cast<VectorArray*>(ret.data());
                auto src_data =
    ...
    for (auto field_id : schema->get_field_ids()) {
        ...
        switch (field_meta.get_data_type()) {
            ...
            case DataType::VECTOR_ARRAY: {
                auto dim = field_meta.get_dim();
                vector<VectorArray> vector_array(N);
                for (int i = 0; i < N / repeat_count; ++i) {
                    milvus::proto::schema::VectorField field_data;
    ...
    if (field_meta.is_vector()) {
        switch (field_meta.get_data_type()) {
            ...
            case DataType::VECTOR_ARRAY: {
                ...
                auto dim = field_meta.get_dim();
                std::vector<ArrayVector> data_raw(src_data.size());
                for (int i = 0; i < src_data.size(); i++) {
                    data_raw[i] = ArrayVector(src_data.at(i));
                }
