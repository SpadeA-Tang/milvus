...
// Simple Slice class to replace RocksDB's Slice
class Slice {
 public:
    Slice() : data_(""), size_(0) {
    }
...
class SyncPoint {
 public:
    ...
    void
    LoadDependencyAndMarkers(const std::vector<SyncPointPair>& dependencies,
    ...
    void
    SetCallBack(const std::string& point,
...
#define TEST_IDX_SYNC_POINT(x, index) \
    milvus::SyncPoint::GetInstance()->Process(x + std::to_string(index))
#define TEST_SYNC_POINT_CALLBACK(x, y) \
    milvus::SyncPoint::GetInstance()->Process(x, y)
