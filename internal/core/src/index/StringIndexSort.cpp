...
void
StringIndexSort::BuildNestedForUT(
    size_t n, const boost::container::vector<std::string>* arrays) {
    if (is_built_)
        return;
    if (n == 0) {
        ThrowInfo(DataIsEmpty, "StringIndexSort cannot build null values!");
    ...
    // Convert boost::container::vector<std::string>* to FieldDataPtr
    std::vector<Array> array_data;
    array_data.reserve(n);
    for (size_t i = 0; i < n; ++i) {
        proto::schema::ScalarField field_data;
        for (size_t j = 0; j < arrays[i].size(); ++j) {
            field_data.mutable_string_data()->add_data(arrays[i][j]);
        }
        array_data.emplace_back(field_data);
    }

    auto field_data =
        storage::CreateFieldData(DataType::ARRAY, DataType::STRING, false);
