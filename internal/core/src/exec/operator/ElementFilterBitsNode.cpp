...
PhyElementFilterBitsNode::GetOutput() {
    ...
    std::cout << "debug=== ElementFilterBitsNode: time cost: "
              << total_cost / 1000 << " ms" << std::endl;
...
PhyElementFilterBitsNode::EvaluateElementExpression(
    ...
    std::cout << "debug=== ElementFilterBitsNode: doc_hit_ratio: "
              << doc_hit_ratio << " count: " << doc_bitset.count()
              << " size: " << doc_bitset.size() << std::endl;
