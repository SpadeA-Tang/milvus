...
PhyVectorSearchNode::GetOutput() {
    ...
    // When embedding search embedding on embedding list is used, which means is_element_level_ is true, we need to transform doc-level
    ...
    if (milvus::exec::UseVectorIterator(search_info_) && ph.is_element_level_) {
