...
SegmentGrowingImpl::InitializeArrayOffsets() {
    ...
    // Create one GrowingArrayOffsets per struct, shared by all its fields
    for (const auto& [struct_name, field_ids] : struct_fields) {
        auto array_offsets = std::make_shared<GrowingArrayOffsets>();
        ...
        // Map all field_ids from this struct to the same GrowingArrayOffsets
        ...
        LOG_INFO(
            "Created GrowingArrayOffsets for struct '{}' with {} fields, "
