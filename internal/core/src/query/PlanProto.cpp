    ...
    if (anns_proto.has_predicates()) {
        ...
        if (is_iterative) {
            ...
            if (doc_expr) {
                ...
                // Handle materialized view (only for doc-level filters)
                if (!is_element_level &&
        ...
        // No filter: build simple vector search plan
...
ProtoParser::ParseUnaryRangeExprs(const proto::plan::UnaryRangeExpr& expr_pb) {
    ...
    if (column_info.is_element_level()) {
        // For element-level access: schema must be ARRAY, element_type must match column_info.data_type
        Assert(data_type == DataType::ARRAY);
        Assert(field.get_element_type() ==
               static_cast<DataType>(column_info.data_type()));
    } else {
        // For document-level access: schema data_type must match column_info.data_type
