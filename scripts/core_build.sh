...
-S: enable syncpoint for testing(default: OFF)
-h: help

usage:
./core_build.sh -p \${INSTALL_PREFIX} -t \${BUILD_TYPE} -s \${CUDA_ARCH} -f \${TANTIVY_FEATURES} [-u] [-l] [-r] [-c] [-z] [-g] [-m] [-e] [-h] [-b] [-o] [-S]
                "
    exit 0
    ;;
  ?)
...
-DENABLE_SYNCPOINT=${ENABLE_SYNCPOINT} "
