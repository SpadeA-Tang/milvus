...
func (s *InsertSuite) TestInsertStorageV2() {
	...
	schema := integration.ConstructSchemaWithStructField(collectionName, dim, false)
	marshaledSchema, err := proto.Marshal(schema)
	s.NoError(err)
	...
	hashKeys := integration.GenerateHashKeys(rowNum)
	structFieldData := integration.NewStructFieldData(schema.StructFields[0], "structField", rowNum, dim)

	// insert
	insertResult, err := c.Proxy.Insert(ctx, &milvuspb.InsertRequest{
		DbName:         dbName,
		CollectionName: collectionName,
		FieldsData:     []*schemapb.FieldData{pkFieldData, vecFieldData, structFieldData},
		HashKeys:       hashKeys,
		NumRows:        uint32(rowNum),
	})
	s.NoError(err)
